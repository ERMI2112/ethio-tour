...
<div class="container-fluid py-4 py-lg-5 px-3 px-lg-5">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4 pb-3 border-bottom"><div><p class="text-uppercase text-muted small fw-semibold mb-1">Event organizer portal</p><h1 class="h3 mb-1">{{ $provider->business_name }}</h1><p class="text-secondary mb-0">{{ $provider->user?->email }}</p></div><div class="d-flex gap-2"><a class="btn btn-dark btn-sm" href="{{ route('event-organizer.events.create') }}">Create event</a><a class="btn btn-outline-secondary btn-sm" href="{{ route('event-organizer.profile') }}">Profile</a></div></div>
