    ...
    <div class="row g-4">
        ...
        <div class="col-lg-9">
            <div class="ws-page-header mb-4">
                <div>
                    <span class="ws-eyebrow"><span class="ws-eye-dot" aria-hidden="true"></span>Tour Guide Portal</span>
                    <h1 class="ws-title">Welcome back, {{ $guide->user?->email }}</h1>
                    <p class="ws-lead">License {{ $guide->license_number }} · Daily rate: {{ $guide->daily_rate === null ? 'Not set' : number_format((float) $guide->daily_rate, 2).' ETB' }}</p>
                </div>
                <div class="ws-actions"><a class="btn btn-primary" href="{{ route('tour-guide.profile.edit') }}">Edit profile</a></div>
            </div>
            ...
            <section aria-labelledby="attention-heading" class="mb-4"><h2 id="attention-heading" class="ws-section-title mb-3">Needs attention</h2>@if($stats['pendingRequests'] > 0)<div class="alert alert-warning d-flex flex-wrap justify-content-between align-items-center gap-3"><div><strong>{{ $stats['pendingRequests'] }} booking request(s) waiting</strong><div class="small">Review dates and availability before deciding.</div></div><a class="btn btn-warning" href="{{ route('tour-guide.requests.index', ['status'=>'pending']) }}">Review requests</a></div>@elseif($guide->verification_status !== 'verified')<div class="alert alert-info">Your profile is awaiting Bureau verification. Verification decisions are read-only here.</div>@else<div class="alert alert-success"><strong>No booking requests are waiting.</strong> Check Availability before accepting new tours.</div>@endif</section>
            <div class="row g-3 mb-4">
                <div class="col-6 col-xl-3"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="ws-stat-label">Pending guide requests</div><div class="ws-stat-value">{{ $stats['pendingRequests'] }}</div></div></div></div>
                <div class="col-6 col-xl-3"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="ws-stat-label">Active guide bookings</div><div class="ws-stat-value">{{ $stats['activeBookings'] }}</div></div></div></div>
                <div class="col-6 col-xl-3"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="ws-stat-label">Completed bookings</div><div class="ws-stat-value">{{ $stats['completedBookings'] }}</div></div></div></div>
                <div class="col-6 col-xl-3"><div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="ws-stat-label">Average rating</div><div class="ws-stat-value">{{ $stats['averageRating'] === null ? '—' : number_format((float) $stats['averageRating'], 1) }}</div><small class="text-muted">{{ $stats['reviewCount'] }} {{ $stats['reviewCount'] === 1 ? 'review' : 'reviews' }}</small></div></div></div>
            </div>
