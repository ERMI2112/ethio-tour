@extends('layouts.app')

@section('title', 'Hotel & Provider Application Profile · ' . $provider->business_name)

@section('content')
<div class="container-fluid py-4 py-lg-5 px-3 px-lg-5">
    {{-- Header & Stepper Indicator --}}
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4 pb-3 border-bottom">
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1 small">
                    <li class="breadcrumb-item"><a href="{{ route('provider.status') }}" class="text-success text-decoration-none fw-semibold">Application Status</a></li>
                    <li class="breadcrumb-item active text-muted" aria-current="page">Provider Profile</li>
                </ol>
            </nav>
            <div class="d-flex align-items-center gap-2 mb-1">
                <span class="badge bg-success-subtle text-success border border-success-subtle rounded-pill px-2.5 py-0.5" style="font-size: 0.72rem; font-weight: 700;">
                    🏨 Official Provider Application
                </span>
                <span class="badge bg-light text-dark border rounded-pill px-2.5 py-0.5 font-monospace" style="font-size: 0.72rem;">
                    ID #APP-{{ str_pad($provider->provider_id, 4, '0', STR_PAD_LEFT) }}
                </span>
            </div>
            <h1 class="h3 fw-bold mb-0 text-dark" style="font-family: var(--font-display); letter-spacing: -0.02em;">
                Provider Profile
            </h1>
            <p class="text-secondary mb-0 small">
                Complete your property credentials, hotel manager details, and operating specifications for Tourism Bureau verification.
            </p>
        </div>

        <div class="d-flex align-items-center gap-2">
            <a class="btn btn-outline-secondary btn-sm rounded-pill px-3.5 py-2 fw-semibold" href="{{ route('provider.status') }}">
                &larr; Return to Application Status
            </a>
        </div>
    </div>

    @include('layouts.partials.flash-messages')

    <form method="POST" action="{{ route('provider.profile.update') }}">
        @csrf
        @method('PUT')

        <div class="row g-4">
            {{-- Left Column (8 cols): The 4 Application Modules --}}
            <div class="col-lg-8">
                {{-- Module 1: Business & Property Identity --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white p-3.5 border-bottom d-flex align-items-center gap-2.5">
                        <div class="rounded-circle bg-primary-subtle text-primary d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            <i class="bi bi-building fs-6"></i>
                        </div>
                        <div>
                            <h2 class="h6 fw-bold mb-0 text-dark" style="font-family: var(--font-display);">1. Property &amp; Business Identity</h2>
                            <span class="text-muted small" style="font-size: 0.75rem;">Official hotel name, jurisdiction, and hospitality classification</span>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-7">
                                <label class="form-label small fw-bold text-dark" for="business_name">Business name <span class="text-danger">*</span></label>
                                <input class="form-control rounded-3 @error('business_name') is-invalid @enderror" id="business_name" name="business_name" value="{{ old('business_name', $provider->business_name) }}" required placeholder="e.g. Loza Hotel Gondar">
                                @error('business_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-5">
                                <label class="form-label small fw-bold text-dark" for="star_rating">Hospitality Classification</label>
                                <select class="form-select rounded-3 @error('star_rating') is-invalid @enderror" id="star_rating" name="star_rating">
                                    <option value="">Select classification tier...</option>
                                    <option value="5_star" @selected(old('star_rating', $provider->star_rating) === '5_star')>5-Star Luxury Resort</option>
                                    <option value="4_star" @selected(old('star_rating', $provider->star_rating) === '4_star')>4-Star Executive Hotel</option>
                                    <option value="3_star" @selected(old('star_rating', $provider->star_rating) === '3_star')>3-Star Standard Hotel</option>
                                    <option value="heritage_lodge" @selected(old('star_rating', $provider->star_rating) === 'heritage_lodge')>Boutique Heritage Lodge</option>
                                    <option value="eco_lodge" @selected(old('star_rating', $provider->star_rating) === 'eco_lodge')>Eco-Lodge &amp; Nature Retreat</option>
                                    <option value="city_hotel" @selected(old('star_rating', $provider->star_rating) === 'city_hotel')>City Center Commercial Hotel</option>
                                </select>
                                @error('star_rating')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="destination_id">Destination Jurisdiction</label>
                                <select class="form-select rounded-3 @error('destination_id') is-invalid @enderror" id="destination_id" name="destination_id">
                                    <option value="">Select regional destination...</option>
                                    @foreach($destinations as $dest)
                                        <option value="{{ $dest->destination_id }}" @selected(old('destination_id', $provider->destination_id) == $dest->destination_id)>
                                            {{ $dest->name }} {{ $dest->tagline ? '· '.$dest->tagline : '' }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('destination_id')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="physical_address">Physical Address in City</label>
                                <input class="form-control rounded-3 @error('physical_address') is-invalid @enderror" id="physical_address" name="physical_address" value="{{ old('physical_address', $provider->physical_address) }}" placeholder="e.g. Piazza Kebele 02, Near Fasil Ghebbi, Gondar">
                                @error('physical_address')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>
                    </div>
                </div>

                {{-- Module 2: Hotel General Manager & Key Contacts --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white p-3.5 border-bottom d-flex align-items-center gap-2.5">
                        <div class="rounded-circle bg-success-subtle text-success d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            <i class="bi bi-person-badge fs-6"></i>
                        </div>
                        <div>
                            <h2 class="h6 fw-bold mb-0 text-dark" style="font-family: var(--font-display);">2. Hotel General Manager &amp; Management</h2>
                            <span class="text-muted small" style="font-size: 0.75rem;">Verified manager credentials and direct communication channels</span>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="manager_name">General Manager Full Name</label>
                                <input class="form-control rounded-3 @error('manager_name') is-invalid @enderror" id="manager_name" name="manager_name" value="{{ old('manager_name', $provider->manager_name) }}" placeholder="e.g. Ato Abnet Kebede">
                                @error('manager_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="manager_title">Executive Designation</label>
                                <input class="form-control rounded-3 @error('manager_title') is-invalid @enderror" id="manager_title" name="manager_title" value="{{ old('manager_title', $provider->manager_title) }}" placeholder="e.g. General Manager / Operations Director">
                                @error('manager_title')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="manager_phone">Manager Direct Mobile / Phone</label>
                                <div class="input-group">
                                    <span class="input-group-text bg-light small font-monospace">+251</span>
                                    <input class="form-control rounded-end @error('manager_phone') is-invalid @enderror" id="manager_phone" name="manager_phone" value="{{ old('manager_phone', $provider->manager_phone) }}" placeholder="91 876 5432">
                                </div>
                                @error('manager_phone')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="contact_email">Official Reservation Email</label>
                                <input type="email" class="form-control rounded-3 @error('contact_email') is-invalid @enderror" id="contact_email" name="contact_email" value="{{ old('contact_email', $provider->contact_email) }}" placeholder="e.g. reservations@hotel.com">
                                @error('contact_email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>
                    </div>
                </div>

                {{-- Module 3: Legal Compliance & Escrow Settlement Payout --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white p-3.5 border-bottom d-flex align-items-center gap-2.5">
                        <div class="rounded-circle bg-warning-subtle text-warning-emphasis d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            <i class="bi bi-shield-check fs-6"></i>
                        </div>
                        <div>
                            <h2 class="h6 fw-bold mb-0 text-dark" style="font-family: var(--font-display);">3. Legal Verification &amp; Escrow Settlement</h2>
                            <span class="text-muted small" style="font-size: 0.75rem;">Tax registration, trade license, and sovereign escrow bank details</span>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-3">
                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="tin_number">Taxpayer ID (TIN Number)</label>
                                <input class="form-control rounded-3 font-monospace @error('tin_number') is-invalid @enderror" id="tin_number" name="tin_number" value="{{ old('tin_number', $provider->tin_number) }}" placeholder="e.g. 0084920194">
                                @error('tin_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-6">
                                <label class="form-label small fw-bold text-dark" for="trade_license_number">Tourism Trade License #</label>
                                <input class="form-control rounded-3 font-monospace @error('trade_license_number') is-invalid @enderror" id="trade_license_number" name="trade_license_number" value="{{ old('trade_license_number', $provider->trade_license_number) }}" placeholder="e.g. TRD-GDR-2024-8891">
                                @error('trade_license_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="payout_bank_name">Escrow Settlement Bank</label>
                                <select class="form-select rounded-3 @error('payout_bank_name') is-invalid @enderror" id="payout_bank_name" name="payout_bank_name">
                                    <option value="">Select Ethiopian bank...</option>
                                    <option value="Commercial Bank of Ethiopia (CBE)" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Commercial Bank of Ethiopia (CBE)')>Commercial Bank of Ethiopia (CBE)</option>
                                    <option value="Awash Bank" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Awash Bank')>Awash Bank</option>
                                    <option value="Dashen Bank" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Dashen Bank')>Dashen Bank</option>
                                    <option value="Bank of Abyssinia" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Bank of Abyssinia')>Bank of Abyssinia</option>
                                    <option value="Telebirr Business Escrow" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Telebirr Business Escrow')>Telebirr Business Escrow</option>
                                    <option value="Cooperative Bank of Oromia" @selected(old('payout_bank_name', $provider->payout_bank_name) === 'Cooperative Bank of Oromia')>Cooperative Bank of Oromia</option>
                                </select>
                                @error('payout_bank_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="payout_account_number">Bank Account Number</label>
                                <input class="form-control rounded-3 font-monospace @error('payout_account_number') is-invalid @enderror" id="payout_account_number" name="payout_account_number" value="{{ old('payout_account_number', $provider->payout_account_number) }}" placeholder="e.g. 1000192837482">
                                @error('payout_account_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="payout_account_name">Registered Account Name</label>
                                <input class="form-control rounded-3 @error('payout_account_name') is-invalid @enderror" id="payout_account_name" name="payout_account_name" value="{{ old('payout_account_name', $provider->payout_account_name) }}" placeholder="e.g. Loza Hotel PLC">
                                @error('payout_account_name')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>
                    </div>
                </div>

                {{-- Module 4: Operational Capacity, Timing & Amenities --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white p-3.5 border-bottom d-flex align-items-center gap-2.5">
                        <div class="rounded-circle bg-info-subtle text-info-emphasis d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            <i class="bi bi-door-closed fs-6"></i>
                        </div>
                        <div>
                            <h2 class="h6 fw-bold mb-0 text-dark" style="font-family: var(--font-display);">4. Operating Specifications &amp; Amenities</h2>
                            <span class="text-muted small" style="font-size: 0.75rem;">Declared room capacity, check-in schedules, and guest features</span>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="row g-3 mb-4">
                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="total_rooms_count">Total Physical Rooms Capacity</label>
                                <input type="number" min="1" max="5000" class="form-control rounded-3 font-monospace @error('total_rooms_count') is-invalid @enderror" id="total_rooms_count" name="total_rooms_count" value="{{ old('total_rooms_count', $provider->total_rooms_count ?: 32) }}">
                                @error('total_rooms_count')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="check_in_time">Standard Check-in Time</label>
                                <input class="form-control rounded-3 font-monospace @error('check_in_time') is-invalid @enderror" id="check_in_time" name="check_in_time" value="{{ old('check_in_time', $provider->check_in_time ?: '14:00') }}" placeholder="14:00">
                                @error('check_in_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="col-md-4">
                                <label class="form-label small fw-bold text-dark" for="check_out_time">Standard Check-out Time</label>
                                <input class="form-control rounded-3 font-monospace @error('check_out_time') is-invalid @enderror" id="check_out_time" name="check_out_time" value="{{ old('check_out_time', $provider->check_out_time ?: '11:00') }}" placeholder="11:00">
                                @error('check_out_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>

                        <label class="form-label small fw-bold text-dark mb-2">Verified Property Amenities Checklist</label>
                        @php
                            $amenitiesList = [
                                'wifi' => ['label' => 'High-Speed Wi-Fi', 'icon' => 'wifi'],
                                'generator' => ['label' => '24/7 Backup Generator', 'icon' => 'lightning-charge-fill'],
                                'breakfast' => ['label' => 'Complimentary Breakfast', 'icon' => 'cup-hot-fill'],
                                'shuttle' => ['label' => 'Airport Shuttle Service', 'icon' => 'airplane-fill'],
                                'coffee' => ['label' => 'Traditional Coffee Ceremony', 'icon' => 'fire'],
                                'pool' => ['label' => 'Swimming Pool', 'icon' => 'water'],
                                'restaurant' => ['label' => 'On-site Restaurant & Bar', 'icon' => 'egg-fried'],
                                'spa' => ['label' => 'Spa & Wellness Center', 'icon' => 'flower1'],
                                'security' => ['label' => '24/7 Security & CCTV', 'icon' => 'shield-lock-fill'],
                                'parking' => ['label' => 'Free Private Parking', 'icon' => 'car-front-fill'],
                            ];
                            $selectedAmenities = (array) old('amenities', $provider->amenities ?: ['wifi', 'generator', 'breakfast', 'security']);
                        @endphp
                        <div class="row g-2">
                            @foreach($amenitiesList as $key => $amenity)
                                <div class="col-sm-6">
                                    <div class="form-check p-2.5 rounded-3 border bg-light-subtle d-flex align-items-center gap-2">
                                        <input class="form-check-input ms-0 me-2" type="checkbox" name="amenities[]" value="{{ $key }}" id="amenity_{{ $key }}" @checked(in_array($key, $selectedAmenities))>
                                        <label class="form-check-label small fw-semibold text-dark cursor-pointer d-flex align-items-center gap-1.5" for="amenity_{{ $key }}">
                                            <i class="bi bi-{{ $amenity['icon'] }} text-success"></i> {{ $amenity['label'] }}
                                        </label>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                </div>

                {{-- Module 5: Property Overview & Description --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white p-3.5 border-bottom d-flex align-items-center gap-2.5">
                        <div class="rounded-circle bg-secondary-subtle text-dark d-flex align-items-center justify-content-center" style="width: 32px; height: 32px;">
                            <i class="bi bi-card-text fs-6"></i>
                        </div>
                        <div>
                            <h2 class="h6 fw-bold mb-0 text-dark" style="font-family: var(--font-display);">5. Property Presentation &amp; Heritage Story</h2>
                            <span class="text-muted small" style="font-size: 0.75rem;">Describe the unique architecture, mountain views, and hospitality values</span>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        <div class="mb-0">
                            <label class="form-label small fw-bold text-dark" for="description">Detailed Property Overview</label>
                            <textarea class="form-control rounded-3 @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="Describe your property, proximity to historic sites (e.g. Fasil Ghebbi castles), room accommodations, dining options, and guest services...">{{ old('description', $provider->description) }}</textarea>
                            @error('description')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                    </div>
                </div>
            </div>

            {{-- Right Column (4 cols): Application Status Card & Actions --}}
            <div class="col-lg-4">
                {{-- Quick Save Actions Sticky Card --}}
                <div class="card border-0 shadow-sm rounded-4 bg-white p-4 mb-4 sticky-top" style="top: 2rem;">
                    <h2 class="h6 fw-bold mb-1 text-dark" style="font-family: var(--font-display);">
                        Submit Application
                    </h2>
                    <p class="small text-muted mb-3">
                        Save and submit your updated hotel manager and property details directly to the Tourism Bureau verification queue.
                    </p>

                    <div class="d-grid gap-2 mb-3">
                        <button class="btn btn-success rounded-pill py-2.5 fw-bold shadow-sm" type="submit">
                            <i class="bi bi-check2-circle me-1.5"></i> Save profile
                        </button>
                        <a class="btn btn-light border rounded-pill py-2 text-muted fw-semibold small" href="{{ route('provider.status') }}">
                            Cancel &amp; Return
                        </a>
                    </div>

                    <div class="p-3 rounded-3 bg-light-subtle border">
                        <div class="small fw-bold text-dark mb-1">
                            <i class="bi bi-shield-check text-success me-1"></i> Sovereign Audit Guarantee
                        </div>
                        <p class="small text-muted mb-0" style="font-size: 0.75rem; line-height: 1.5;">
                            All submitted credentials are reviewed by certified municipal Tourism Bureau officers in accordance with regional hospitality guidelines.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </form>
</div>
@endsection
