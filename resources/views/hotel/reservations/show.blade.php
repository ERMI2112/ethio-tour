@extends('layouts.app')

@section('title', 'Hotel Reservation #BK-' . sprintf('%05d', $booking->booking_id))

@section('content')
<div class="container py-4 py-lg-5">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb small mb-2">
            <li class="breadcrumb-item"><a href="{{ route('hotel.dashboard') }}">Hotel Dashboard</a></li>
            <li class="breadcrumb-item"><a href="{{ route('hotel.reservations.index') }}">Reservations</a></li>
            <li class="breadcrumb-item active" aria-current="page">#BK-{{ sprintf('%05d', $booking->booking_id) }}</li>
        </ol>
    </nav>

    <div class="d-flex justify-content-between align-items-center mt-3 mb-4">
        <div>
            <h1 class="h3 mb-1">Reservation Request #BK-{{ sprintf('%05d', $booking->booking_id) }}</h1>
            <p class="text-muted small mb-0">Received on {{ $booking->booking_date ? $booking->booking_date->format('F d, Y at H:i') : $booking->created_at->format('F d, Y') }}</p>
        </div>
        <div>
            <x-ui.status-badge :status="$booking->status" />
        </div>
    </div>

    @include('layouts.partials.flash-messages')

    <div class="row g-4">
        <div class="col-lg-8">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3">
                    <h2 class="h5 mb-0">Guest & Stay Details</h2>
                </div>
                <div class="card-body p-4">
                    <div class="row g-3 mb-4">
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Tourist Name</span>
                            <strong class="fs-6">{{ $booking->tourist->full_name ?? 'N/A' }}</strong>
                        </div>
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Nationality</span>
                            <strong class="fs-6">{{ $booking->tourist->nationality ?? 'N/A' }}</strong>
                        </div>
                    </div>

                    @php
                        $res = $booking->hotelRoomReservation;
                        $nights = $res ? max(1, (int) $res->check_in_date->diffInDays($res->check_out_date)) : 1;
                        $nightlyPrice = $booking->tourismService->price ?? 0;
                        $totalCost = $nights * $nightlyPrice;
                    @endphp

                    <h5 class="h6 text-muted text-uppercase mt-4 mb-3">Room & Dates</h5>
                    <div class="row g-3 p-3 bg-light rounded border">
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Requested Room Type</span>
                            <strong>{{ $booking->tourismService->service_name ?? 'N/A' }}</strong>
                        </div>
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Guest Count</span>
                            <strong>{{ $res->guest_count ?? 1 }} guest(s)</strong>
                        </div>
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Check-in Date</span>
                            <strong>{{ $res ? $res->check_in_date->format('F d, Y') : 'N/A' }}</strong>
                        </div>
                        <div class="col-sm-6">
                            <span class="text-muted small d-block">Check-out Date</span>
                            <strong>{{ $res ? $res->check_out_date->format('F d, Y') : 'N/A' }}</strong>
                        </div>
                        <div class="col-12 pt-2 border-top">
                            <span class="text-muted small d-block">Physical Room Allocation Status</span>
                            @if ($res && $res->hotelRoom)
                                <strong class="text-success fs-6">Assigned: Room {{ $res->hotelRoom->room_number }}</strong>
                            @else
                                <strong class="text-warning fs-6">Unassigned (will be automatically allocated upon acceptance)</strong>
                            @endif
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-lg-4">
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3">
                    <h2 class="h5 mb-0">Actions & Total</h2>
                </div>
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between mb-2">
                        <span>Nightly Rate</span>
                        <span>{{ number_format($nightlyPrice, 2) }} ETB</span>
                    </div>
                    <div class="d-flex justify-content-between mb-3 pb-2 border-bottom">
                        <span>Duration</span>
                        <span>{{ $nights }} night(s)</span>
                    </div>
                    <div class="d-flex justify-content-between fs-5 fw-bold text-primary mb-4">
                        <span>Estimated Booking Value</span>
                        <span>{{ number_format($totalCost, 2) }} ETB</span>
                    </div>

                    @if ($booking->status === 'pending')
                        <div class="d-grid gap-2">
                            <form method="POST" action="{{ route('hotel.reservations.accept', $booking) }}">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn btn-success btn-lg w-100">Accept Reservation</button>
                            </form>
                            <form method="POST" action="{{ route('hotel.reservations.reject', $booking) }}" onsubmit="return confirm('Are you sure you want to reject this reservation request?');">
                                @csrf
                                @method('PATCH')
                                <button type="submit" class="btn btn-outline-danger w-100">Reject Reservation</button>
                            </form>
                        </div>
                    @else
                        <div class="alert alert-secondary small mb-0 text-center">
                            This reservation is currently <strong>{{ ucfirst($booking->status) }}</strong>.
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
